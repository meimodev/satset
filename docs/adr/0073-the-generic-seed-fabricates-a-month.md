...
## Addendum — the one-row state was read four ways, three of them wrong (2026-08-28)

Decisions 8–11 all hang off a single `demo_states` row, and three separate defects came from readings of that row that disagreed with each other. Recorded here rather than in a new ADR: these correct this decision's own mechanism, and splitting one story across two files is how the second reader misses the first.

- **A skip is not an interrupted job.** `markSkipped` writes the same row `begin` does, so `complete` falls to its column default of `false`. `isIncomplete` was `row != null && !row.complete`, which meant *every* skip read as an interrupted job — and because `phase` checks `seedIncomplete` before `promptAnswered`, decision 8's "asked exactly once" became asked forever. The predicate for "a job actually began" is now `daysTotal > 0`.

- **A clear is an update, not a delete.** `SeedJob.clear` dropped the row, and `promptAnswered` with it — so clearing from decision 9's permanent way back in silently un-asked the question the venue had already answered. It now zeroes the job fields and keeps the answer. `daysTotal > 0` carries the whole "is there a job" load, so no reader depends on row existence any more.

- **A failed job is not the same as an interrupted one, and the verdict must outlive the process.** Decision 10 gave both states the same recovery, which is right, but the *failed* verdict lived only in the `seed.progress` broadcast. Relaunch the app and a crashed job read as merely interrupted — the right dialog with the wrong sentence. `demo_states.failed` (v64) persists it, reset by `begin`, `clear` and `markComplete` so a retry never renders the last attempt's verdict. Rejected: conflating the two permanently, since the recovery is identical — but a flag that only changes a sentence is still worth the column when the alternative is a sentence that is wrong.

- **Decision 10's exit door leaked.** *Lewati* was offered in every phase, including a broken one. Declining out of a half-written month wrote `promptAnswered`, retired the dialog forever, and left partial fabricated bills standing in reports and the audit log — reading as real trade, and having already tripped ADR-0052 §3's guard so nothing could re-seed over them. A skip from a broken phase now clears first. Declining sample data must never leave sample data behind.

- **`canSeedSample` is self-tripping and one-way, and the UI was pretending otherwise.** Once a venue holds a live ticket or an archived session the guard is false permanently, because decision 3's clear deletes by the `contoh-` tag and can never reach a row a real order wrote. The dialog nonetheless offered *Hapus & muat ulang*, whose clear succeeded and whose retry 409'd on every tap. The retry is now conditional on `canSeed` re-read after the clear, and the body says why instead. The guard itself is unchanged — it is protecting a real venue's books, and a "hard reset" button that bypassed it would live one tap from a production till to buy a convenience only a developer wants. On a test device the answer is to clear app data.
