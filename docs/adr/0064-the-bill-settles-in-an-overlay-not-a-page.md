...
- `_CloseBar`'s bottom padding is now wrong if the view is ever mounted
  ...
  one.
- The receipt card stays hand-drawn where the totals and lines cards became
  `SatCard`. This landed beside ADR-0063, which stopped its outline being
  chrome: the border now carries the guest's letter hue, and `SatCard` owns
  its border with no hue axis. The card that folds is the one whose surface
  means nothing.
