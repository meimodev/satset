...
- Server enforces `voidReason` + `voidReasonCode` non-null on `tickets/<id>/transition` when target status is `voided` — UI-only enforcement is no longer sufficient now that the gate is gone.
