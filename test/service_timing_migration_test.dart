...
void main() {
  ...
  test(
    'prep_time relaxes to nullable and keeps deliberate overrides',
    () async {
  ...
  test('a null prep_time is writable and readable end to end', () async {
    ...
    await (db.update(db.menuItems)..where((t) => t.id.equals('inherit'))).write(
      const MenuItemsCompanion(prepTime: Value(12)),
    );
    expect(await prepOf('inherit'), 12);

    // Clearing the override returns the item to inherit.
    await (db.update(db.menuItems)..where((t) => t.id.equals('inherit'))).write(
      const MenuItemsCompanion(prepTime: Value(null)),
    );
    expect(await prepOf('inherit'), isNull);
  });
  ...
  test('firedAt defaults to null and survives a round trip', () async {
    ...
    await (db.update(db.tickets)..where((t) => t.id.equals('t1'))).write(
      TicketsCompanion(status: const Value('sent'), firedAt: Value(firedAt)),
    );
    row = await (db.select(
      db.tickets,
    )..where((t) => t.id.equals('t1'))).getSingle();
