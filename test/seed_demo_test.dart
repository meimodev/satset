...
void main() {
  ...
  test('refuses on a venue that has already traded', () async {
    ...
    await expectLater(seedDemoVenue(db), throwsA(isA<DemoSeedRefused>()));
  ...
  test(
    'reset removes every demo row and repairs balances',
    () async {
      await seedDemoVenue(db);
  ...
  test(
    'reset restores authored availability, not blanket-available',
    () async {
      await seedGenericRestaurant(db);
