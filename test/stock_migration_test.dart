...
void main() {
  ...
  test(
    'legacy item stock counts become pcs ingredients with a 1-pcs recipe',
    () async {
      ...
      final ing = await (db.select(
        db.ingredients,
      )..where((i) => i.id.equals('ing_cola'))).getSingle();
  ...
  test(
    'inventory capabilities backfill from the nearest existing authority',
    () async {
