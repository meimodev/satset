...
A split answers **who pays**. At a venue holding the **`memberSplit`** mode it answers a second question alongside it — **who each share is for** — by naming a [[Pemilik struk]] per receipt (ADR-0118). Same partition, two readings, so they cannot drift apart; they simply differ whenever a guest settles a friend's slip. _Avoid_: reading the payer as the attributee, or the reverse.
...
A [[Pemilik struk]] is the one identity an amount receipt may hold, and only because it is **declared** rather than derived (ADR-0118): a letter is read off the lines a receipt owns, which an even share has none of, whereas a member is named by a [[Cashier]] about a person at the till. The moment one is named the two shares stop being interchangeable, and the share prints that member's name in place of `Bagian 1/3`. _Avoid_: reading this as licence to letter an even share.
...
**Distinct from `guestName`.** The waiter's party label on a [[Visit]] ("Pak Budi, 4 orang") is a *per-visit* string; the Pelanggan is the person behind it. Attaching a member **fills an empty `guestName` and never overwrites** one a waiter typed — the floor recognises the waiter's label, not the CRM's. A member attaches to the **Visit** — where they are the [[Pemilik tagihan]] — and by default that is the only member a bill has: one party, one member, and a [[Split bill]] across three payers must not earn three times over for the same person. A venue holding the `memberSplit` mode may **additionally** name a [[Pemilik struk]] on each receipt (ADR-0118). That never displaces the owner: the visit's member still holds the party's identity on the floor and still takes whatever money no receipt claimed, so the sentence above is exactly what a bill does whenever nobody has said otherwise.
...
The owner is also the **default and the backstop**. Money that no receipt claims is theirs, which is what makes a venue that never splits behave identically to one that has never heard of a [[Pemilik struk]]: an unsplit bill mints no receipts until payment, so the entire bill is unclaimed remainder and the entire earn is the owner's. A bill always has exactly one owner or none; it never has two. _Avoid_: treating the owner as merely "receipt zero" (they exist before any receipt does, and outlive every one of them); reading an unattributed receipt as ownerless rather than as the owner's; letting a [[Split bill]] leave a bill with no owner.

### Pemilik struk
**ID · EN** — Pemilik struk · Receipt owner.

The [[Pelanggan (member)]] one **receipt** is *for* — `receipts.memberId`, live only at a venue holding the **`memberSplit`** mode (ADR-0118). It is the answer to *who is this share for*, which is a different question from the one [[Split bill]] already answers, *who pays this share*: a receipt names both, and a guest settling their friend's slip makes them differ.

Attribution rides the **receipt** and not the ticket line, deliberately — splitting the bill *is* splitting the attribution, so the two partitions can never disagree, and a receipt already carries the `total`, `serviceAmount` and `taxAmount` a [[Poin]] earn needs. Both kinds of receipt may hold one, an [[Amount receipt]] included (see there: declared, never derived). What the owner gets: [[Poin]] on their own share's money, [[Stempel]] on their own share's lines, their tier [[Diskon (discount)]] and any [[Tukar poin (redeem)|redemption]] against their own receipt rather than the bill, and — since ADR-0120 — their own [[Piutang]] charged when that share is paid on account, against their own credit limit. Their name replaces the share's label on the slip they are handed.

It is **declared once and frozen with the money** — set while the receipt is unpaid, locked at its first payment along with everything else on it (ADR-0068), corrected only through the audited [[Settlement (recording payment)|reopen]]. It is a **weak reference**, like the owner's: deleting a member leaves it dangling on purpose, because the receipt *was* theirs. Switching the mode off **freezes rather than deletes** — stored owners stay, the step disappears, earn falls back to the [[Pemilik tagihan]], and an already-attributed window keeps reporting as attributed. _Avoid_: a receipt owner instead of a bill owner; attributing a ticket line independently of its receipt; re-attributing a paid receipt without reopening it; reading "who pays" off this field.
...
Under the **`memberSplit`** mode the moment is unchanged and only the *grain* moves (ADR-0118): a close writes one earn row per [[Pemilik struk]], each on its own receipt's money by the same net-of-discount, before-service-and-tax formula, plus one for whatever the [[Pemilik tagihan]] was left holding. Which means a reopen must reverse **every** row the close wrote — reversing one of four is a bill that keeps three guests paid.

**Points never expire** (ADR-0095) — the venue server has no scheduler, and a balance that changes by being looked at is worse than a growing one. The honest substitute is **Poin beredar** in the members Reports section: the outstanding balance read as a **liability**. _Avoid_: storing a balance; earning per **payment** (a receipt may take several, and a reopen would then earn twice); earning per receipt at a venue **without** the `memberSplit` mode, or per receipt for a member who is only the [[Pemilik tagihan]]; earning on service and tax; expiring points on read; a negative balance.
...
An owner-run "buy N, get one free" program on **one menu item at a time**, live only when **`memberPunchEnabled`** is on. Progress is **derived from settled history** — paid, non-voided, non-[[Comp|comped]] units of that item on closed bills — never a stored counter, for the same reason the cash balance is not stored. The **reward is a [[Comp]]**, offered at settlement and booked through the existing comp path, so it reports as a write-off rather than as revenue and is already audited; **the free unit does not count** toward the next card. Takeaway counts — same guest, same item. Under the **`memberSplit`** mode the units counted are the ones on that member's own receipts rather than on every bill they owned, at the same qty grain the [[Split bill]] assigns them (ADR-0118) — a shared `qty: 3` plate divided 2+1 punches two cards twice and once, not one card three times. _Avoid_: many concurrent programs (that is a promo engine); a stored punch counter; booking the reward as a discount; counting comped or voided units; flattening a split line onto whoever owned the bill.
