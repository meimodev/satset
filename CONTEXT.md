...
**A lease that cannot be asked for is not a lease held by someone else.** A [[Terputus|terputus]] handset cannot acquire the lease — the request needs the host — so "no lease" has two meanings that must not be collapsed: *somebody else has this table*, which blocks everything, and *nobody could be asked*, which blocks only what the [[Antrean setelmen]] cannot carry. In the second, taking an order, voiding a line and **settling the bill** stay open, because all three have a chain behind them (ADR-0139); firing a [[Course|hidangan]], serving, and editing the cover count wait, because none of them does. The lease is never faked, claimed or heartbeaten while terputus — the host still arbitrates on drain — and the padlock keeps its one meaning: *someone else has this table*, never *the network is down*. See ADR-0116. _Avoid_: reading the padlock as an outage, or offering an offline control whose act nothing can queue.
...
- **Lunas** (normal) — an explicit cashier confirmation ends a fully paid bill. A fully paid bill may remain open, retaining its payments and accepting further items until the cashier closes it. See [ADR-0138](docs/adr/0138-paid-bills-close-only-with-confirmation.md).
- **Tak tertagih** (write-off) — ends an unpaid or under-paid bill as a recorded loss. Requires a reason and manager approval; the loss remains distinct from a comp.
...
A void may be **captured rather than delivered**: it is the one move on the ticket graph that a terputus handset parks on the [[Antrean setelmen]] (ADR-0114), because a guest changes their mind in exactly the corner where the signal dies. A captured void is a real void on that handset and **no void at all to the kitchen** until the queue drains — the dish may still be plated in the meantime, and the screen says so. It is also the one intent whose failure leaves something *live*: a stranded order never happened, a stranded void leaves the line on the guest's bill. _Avoid_: reading a queued void as done; giving any other ticket transition an offline path (a replayed `prep` announces a kitchen fact that stopped being true minutes ago).
...
One named thing a person is allowed to do — `takeOrder`, `refund`, `overrideStock`. Twenty-three of them, fixed in code, each belonging to exactly one of five **groups** that exist only to organise the reading. A capability is never held by a person: it is held by a [[Role]], and a person inherits it by wearing that role. Most decide route access (see `_capabilityFor` in the router); the rest gate actions inside a screen. The **local server is the authority** on every one of them — Firebase carries identity, never capabilities.

**Every one of them gates a write** ([ADR-0132](docs/adr/0132-a-grantable-capability-must-gate-something.md)). The role sheet renders each as a labelled toggle with a sentence saying what it does, so a capability no route asks for is not inert — it is a switch an owner ticks believing they configured something. Two were: `manageRoles`, which role CRUD never demanded, and `adjustStock`, which v36 superseded into `manageIngredients` and nothing removed. Both now gate what their copy already claimed. Stock is cut between the **catalogue** (`manageIngredients` — author a bahan and its batch recipe) and the **ledger** (`adjustStock` — receive, waste, produce, and walk an [[Opname (Stocktake)|opname]] from open to close); neither implies the other, and reads take either. Rewriting a role's permission set costs `manageRoles` on top of `manageStaff`, on the create path as well as the edit path — an ungated create is the same act by another name. _Avoid_: adding a capability without the gate in the same commit; gating a screen on the `UserRole` bucket, which reads `manageStaff || manageRoles || editSettings` and so hands out authorities the ticked switch does not name.
...
### Antrean kirim (send queue) — retired
**ID · EN** — Antrean kirim · Send queue. **Retired by ADR-0139; use [[Antrean setelmen]].**

Was the device-local FIFO of **intents** a [[Terputus (client disconnected)|terputus]] handset had captured but not delivered — "seat this table", "send these lines" — held separately from the money queue. ADR-0139 merged the two: a seat, an order, a void and an expense are now events on the same per-[[Visit]] chain as the payment, because the seat must land before the order and the order before the payment, and two backlogs with independent sequences cannot carry one causal chain. The name survives only to resolve older writing and older code. _Avoid_: using it for the merged queue; "outbox", "sinkron", "rekonsiliasi".

### Pesanan tertunda (pending order)
**ID · EN** — Pesanan tertunda · Pending order.

A line captured on a [[Terputus (client disconnected)|terputus]] device and not yet accepted by the host — see [[Baris tertangkap]], which is what it *is*. What stays provisional is narrow and worth naming exactly: no [[KDS / Antrian Persiapan|KDS]] has seen it and no stock has moved. Everything else it once lacked, it now has — a client-minted ticket id, a [[Visit]] that owns it, a place on the [[Bill (tab)|bill]], and a price the guest can pay (ADR-0139). The waiter may still edit or void their own; once delivered it is an ordinary sent line and [[Void (item)|the freeze rule]] applies. _Avoid_: rendering it as "Terkirim"; **keeping it out of the bill** — until ADR-0139 a bill deliberately could not reach it, which is how a guest ate five plates and paid for two.

### Hasil pengiriman (send result)
**ID · EN** — Hasil pengiriman · Send result. The failure list within it: Gagal terkirim · Failed to send.

What the host answered when the [[Antrean setelmen]] drained: accepted, refused (out of stock, the [[Visit]] closed, the table changed guests), or expired past the business-day boundary. A clean drain passes silently; anything refused is shown and **acknowledged**, then survives on the [[Shift|Saya]] tab until resolved. _Avoid_: silently dropping a refusal, or retrying one automatically — a line the host refused is a fact the waiter must act on, not a transport error.
...
The device-local, **per-[[Visit]] ordered** log of everything a [[Terputus (client disconnected)|terputus]] device has captured but not yet delivered. Since ADR-0139 that is the whole visit, not just the money: seat the table, send lines, void a line, file a [[Pengeluaran kunjungan]], mint a receipt, assign, apply a [[Diskon (discount)]], take a [[Payment (manual confirmation)|payment]], close. It is a **chain, not a FIFO of independents** — each event reads what the last one wrote, which is why the seat, its orders and its payment cannot live in separate queues — and it is **read as well as drained**, because the kasir needs the outstanding and the change. Lives in the client database (ADR-0124), not prefs. Expiry is **chain-scoped**: a business-day rollover retires a captured order, unless money is anywhere in that visit's chain, in which case nothing in it expires (ADR-0139, ADR-0130's rule for spent cash). It carries **member acts too** — an offline enrol, an attach, a redeem — and an enrol has no [[Visit]], so visit-less acts share a **venue-scope chain** drained ahead of the per-visit ones (ADR-0129). See [docs/adr/0123-an-offline-settlement-is-a-journal-not-an-intent.md](docs/adr/0123-an-offline-settlement-is-a-journal-not-an-intent.md) and [docs/adr/0139-a-captured-visit-is-authoritative-until-it-drains.md](docs/adr/0139-a-captured-visit-is-authoritative-until-it-drains.md). _Avoid_: "sinkron" / "rekonsiliasi"; letting an event expire out of a chain that holds money; treating it as a queue of independents.
...
A [[Visit]] whose [[Antrean setelmen]] is non-empty. Every act on it **appends to the journal** rather than going to the host — *even after the socket returns* — until its chain drains clean, because a live write that lands ahead of the queued events makes the projection the kasir is reading a lie. The condition is `journal.isNotEmpty || wsConnState != open`, so it outlives the outage that started it. Since ADR-0139 a captured **order** earns authority the same as money does, which needs no second predicate precisely because orders and money now share one store — if the test needs widening, the merge was not done. **Member acts remain the exception**: a member act moves no money and earns no authority, or a pelayan tagging a regular at order time would make the [[Visit]] authoritative on their handset instead of the till's (ADR-0129). _Avoid_: reading "back online" as "back to normal" for a visit that still has events parked; interleaving a live write with an undrained chain.

### Kunjungan tertangkap (captured visit)
**ID · EN** — Kunjungan tertangkap · Captured visit. The badge: Tertangkap · Captured.
...
A line keyed onto a [[Kunjungan tertangkap]] and not yet delivered. It is an **ordinary bill line in every respect the guest can see** — it has a ticket id, a price, a [[Course|hidangan]], a place on the [[Bill (tab)]] and a share of the total — and provisional in exactly two: no [[KDS / Antrian Persiapan|KDS]] has seen it and no stock has moved. The badge names the one fact that still differs, that the host has not seen it, and is the *only* thing that should distinguish it visually. At drain it is **not refused for stock** — the guest ate it and paid for it, so refusing argues with the past; the shortfall goes negative and lands in the [[Audit]] for [[Stok opname|opname]] to close. It carries the **price it was captured at**, not the menu's current one. _Avoid_: repricing it at drain (the drawer will not foot); a second visual vocabulary for it; the phrase "belum jadi pesanan" — it is an order, the host is simply late to hear about it.
...
The on-device copy of the **floor itself** — zones, [[Table|meja]], the [[Ticket]] lines the host has acknowledged, and the menu snapshot — written as the device sees them change, so a [[Terputus (client disconnected)|terputus]] handset that is *restarted* still knows what the venue is. The failure it answers is the **cold boot**, not the disconnection: the repositories live as long as the app, so a device that loses its host mid-shift keeps the floor in memory; kill the process and it used to reopen to an empty venue, an empty menu, and a [[Antrean setelmen]] full of orders aimed at tables it could no longer name. A *copy*, never a source, and never a permission: it decides nothing about what may be written — the queue and [[Kunjungan otoritatif-lokal]] already answer that — and the host's next answer replaces it wholesale. It **never expires**, because a floor from yesterday is wrong about who is sitting where but right about what tables exist, and that second half is the half a [[Waiter|pelayan]] cannot reconstruct; the volatile half is confessed instead, by a single stamped notice on the floor and [[Pesanan board|pesanan]] screens. Dies with the certificate, not the address (ADR-0128), and is not kept by a device running as the host — a copy whose invalidation story is "the paired certificate changed" is nonsense on the device holding that certificate. See [docs/adr/0133-the-floor-travels-with-the-device.md](docs/adr/0133-the-floor-travels-with-the-device.md). _Avoid_: calling it a cache and reasoning about eviction; expiring it at the business-day rollover; letting it gate a write; reading a frozen sold-out flag as the kitchen's current answer; stamping every tile instead of the screen.

### Salinan pelanggan (member mirror)
**ID · EN** — Salinan pelanggan · Member mirror.

The on-device copy of the [[Pelanggan (member)]] directory, filled ahead of an outage so a [[Terputus (client disconnected)|terputus]] device can still **find** a regular and **attach** them — to a [[Ticket]] or a [[Pemilik struk|struk]] — with no host to ask. A *copy*, never a source: the host stays authoritative, every derived figure ([[Poin]], visit count, [[Kartu stempel (punch card)|stempel]] progress, [[Piutang]]) arrives already computed and is shown **with the timestamp it was true at**, and nothing on the device sums a ledger it holds one page of. It carries the **whole** record on a device that may settle a bill, and a **masked** one — the number as a hash plus its last four digits — on a device that may only take orders, matching the masking `/members/lookup` already does on the wire. Dies with the certificate, not the address (ADR-0128) — and with the venue's answer, since a sync that 404s takes the copy with it — never on shift change, and switchable off by the owner as a plain venue setting. See [docs/adr/0129-the-member-directory-travels-with-the-device.md](docs/adr/0129-the-member-directory-travels-with-the-device.md). _Avoid_: treating it as the directory (it is a copy of one); rendering a balance from it without its stamp; printing a points block off it; calling the venue switch a [[Modul]].

### Pendaftaran terlipat (folded enrolment)
**ID · EN** — Pendaftaran terlipat · Folded enrolment.

A member enrolled on a dark device whose phone number turned out to already be in the directory — two islanded tills signing up the same walk-in, or a guest who was a member all along. Because the number **is** the identity (ADR-0092), this is not a clash to refuse but the same person arriving twice: at drain the host **folds** the arrival into the standing record, answers with the surviving id, and the [[Salinan pelanggan]] rewrites its own. Audited under its own kind, never as a human's merge, so the owner can tell "the queue reconciled these" from "someone chose to merge these". Not a contradiction, so it never halts a chain — [[Hasil setelmen]]'s rule stands. _Avoid_: calling it a duplicate (nothing was duplicated once it lands); auditing it as an ordinary merge; letting the client-minted id outlive the fold.
...
Prints at **three granularities**: the **whole-bill** document (the table's entire undivided tab), a **per-receipt** document (one [[Split bill]] receipt), and the [[Rincian pilihan]] (a Per item selection that has not been minted into a receipt yet — the only one of the three backed by nothing stored). An **itemized** receipt lists only *its own* assigned lines with prices; an **even** receipt shows its flat **share** amount plus a compact, price-less **reference list** of the whole table's items (it owns no specific items). Reuses the existing two-scope [[Printer (scope × transport)|printer]] picker, shared print path, and transport rules (ADR-0020/0022) — only the renderer template is new. _Avoid_: overloading the word "Struk" for the money document; handing one combined whole-bill slip to guests who asked for **separate** receipts instead of printing one per receipt.

### Rincian pilihan
**ID · EN** — Rincian pilihan · Selection slip; Tagihan sementara · Provisional bill.

The [[Tagihan / Struk pembayaran|money document]] printed for a **[[Split bill|Per item]] selection the [[Cashier]] has tapped but not yet confirmed** — the guest checks "these are mine" before any money moves (ADR-0122). It is a [[Tagihan / Struk pembayaran|Tagihan]] by the ordinary rule, since a selection carries no [[Payment (manual confirmation)|payment]] for the state to read.

**Ephemeral, and that is the whole point.** Nothing is minted, so a guest who changes their mind leaves nothing behind — the property [[Split bill|mode-per-payment]] exists to preserve. Printing it twice yields two identical slips, and the selection survives the print, so the confirm button beneath it charges exactly what the guest was handed. It states its own provisional standing where a minted share prints its [[Receipt letter]], because the one thing it must never be read as is evidence that a receipt exists.

Its total is the tapped lines grown by the bill's own effective [[Tax & service charge|service and tax]] rate — the same figure the confirm button shows, from one shared rule, so the two can never disagree. It quotes any **line** [[Diskon (discount)]] the [[Cashier]] has already put on a tapped line, because since ADR-0126 those are picked before the receipt exists and the confirm button already has them off — a slip quoting more than the till is the one the guest queries. It still carries **no whole-order or bill discount**: those attach to a receipt and there is none yet, so one applied afterwards only makes the guest's real share smaller than the quote. _Avoid_: reading one as a minted [[Split bill|receipt]]; giving it a [[Receipt letter]]; expecting N of them to sum to the bill total to the rupiah (each rounds on its own — they are quotes, and the minted receipts are what the arithmetic closes on); minting an unpaid receipt to get a printout, or to hang a line discount on.
...
A split answers **which receipt pays for which claim** — and only that. At a venue holding the **`memberSplit`** mode, each [[Pemilik tiket]] answers who consumed a line, and a `piutang` [[Payment (manual confirmation)|payment]] separately names who accepted that debt; the receipt itself names nobody (ADR-0126 retired [[Pemilik struk]]). The facts differ deliberately: Ani may pay a receipt containing Budi's Tickets without taking Budi's loyalty attribution. _Avoid_: deriving consumption or debt responsibility from a receipt; a second member gesture at receipt scope.

### Amount receipt
**ID · EN** — Bagian · Part (`Bagian 1/3` · `Part 1/3`). Under ADR-0085 this is composed from ARB at read time, not stored as a sentence.

A **receipt that owns no lines** and instead holds a frozen **money claim** on the part of a [[Bill (tab)]] no itemized receipt has claimed. It is what an even share *is*: "one third of whatever is left", fixed at the moment it is minted so later edits to the bill cannot silently move what a guest was quoted. The remainder it draws from is the bill total less every itemized receipt's total and every other amount receipt's claim, which is what lets a single bill carry both kinds at once (see [[Split bill]]). A bill is **fully assigned** when every unit is owned by an itemized receipt **or** covered by an amount receipt — the predicate that in turn decides whether the fully paid bill is eligible for [[Bill close (Tutup tagihan)|confirmed closure]]. See [docs/adr/0068-an-even-receipt-is-an-amount-receipt.md](docs/adr/0068-an-even-receipt-is-an-amount-receipt.md).

Shares are rounded so a guest is handed a **note-friendly figure**: each head rounds up to the nearest Rp 100 and the resulting surplus is spread back across the later heads, so nobody is asked for coins and nobody ends up owing nothing. _Avoid_: giving an amount receipt a [[Receipt letter]], or **deriving** any per-guest identity from it (it has none — see there); attaching a line [[Diskon (discount)]] to one; recomputing its claim after it is minted.

An amount receipt holds **no identity at all** since ADR-0126 retired [[Pemilik struk]]: it owns no lines, so it has no [[Receipt letter]] and no [[Pemilik tiket]] to read one off, and the declared label that used to print in place of `Bagian 1/3` is gone. Shares stay interchangeable, which is what ADR-0063 collapsed them into one card for. A bill split evenly attributes its loyalty through the Tickets underneath, exactly as an itemized one does. _Avoid_: lettering an even share; reviving a declared member on one; reading `Bagian 1/3` as a person.
...
A deliberate, authorized reduction of what a guest owes, applied by the [[Cashier]] at [[Settlement (recording payment)|settlement]] and **never** during order-taking. Three targets, one mechanism: a **bill** discount against the whole [[Bill (tab)]], a **whole-order** discount against one receipt, or a **line** discount against one bill line. The bill scope is what a table-wide promo actually is — it reduces the bill subtotal before any receipt claims anything, then distributes across receipts the way an order discount already does, so "20% off the table, split three ways" is one act rather than one per payer. See [docs/adr/0070-discounts-have-a-bill-level-scope.md](docs/adr/0070-discounts-have-a-bill-level-scope.md). Every target carries a **[[Sumber diskon (discount source)|source]]** (`manual` | `member` | `redeem`). **Every** target stacks one row per source (ADR-0126), a line included: a [[Pemilik tiket]]'s automatic member tier and the cashier's manual promo sit side by side on the same dish rather than one locking the other out — locking it out punished the member for being one. The rows are clamped **as a stack** against the line's own base, so 10% + 100% takes the dish to zero and never below; each row still prints what it promised. The member tier is always line-scoped and percentage-based; a fixed member preset cannot be enabled under `memberSplit`, because repeating a fixed amount per Ticket silently multiplies the benefit. Applied rows snapshot their name, kind, and value, so later preset edits never rewrite settled history. Free to add, swap, and remove while the containing receipt is unpaid; **frozen once paid**, corrected only by audited [[Settlement (recording payment)|reopen]]. Manual discounts remain gated by `applyDiscount` or manager step-up; automatic member discounts ride `settleBill`. A discount can never push a total below zero and line discounts remain pre-tax. _Avoid_: a second row of the same source on one target; clamping line rows individually rather than as a stack; multiplying a fixed member benefit per Ticket; letting a payer's tier replace the Ticket owner's.
...
A cashier-recorded **attestation** that a receipt's claim was **discharged** — there is no payment gateway, so recording the payment *is* the confirmation (no verification, no external call). Discharged usually means money changed hands; under `piutang` it means the claim moved onto a [[Pelanggan (member)]]'s [[Piutang]] ledger instead (ADR-0098). The table has never been a record of cash arriving — a **Refund** is stored as a *negative* payment — so it is money **events** against a claim, and `paid` means "this receipt no longer claims anything". A Payment attaches to a **receipt** (not the whole [[Bill (tab)]]) and carries a **method** (`tunai` | `kartu` | `qris` | `transfer` | `piutang` | `lainnya`, the last with a free-text note), an **amount**, the recording **cashier's userId**, and a timestamp. A receipt may hold **multiple payments** (split tender, e.g. part Tunai + part Kartu, or part cash + part on account); it flips to **paid** once `sum(payments) ≥ receipt total`. A receipt is **binary paid/unpaid** — no partial-paid limbo even mid-tender. For **Tunai**, the cashier may enter **amount tendered** and the app shows **change** (`tendered − total`); tendered/change are informational (printed, not stored as revenue — the recorded amount is the receipt total). **Piutang** is the one method that is neither cash nor proven: every `piutang` leg explicitly names the member accepting that debt, carries no tendered/change or [[Payment proof photo (Bukti pembayaran non-tunai)|proof photo]], and requires headroom under that named member's credit limit. One receipt may carry separate `piutang` legs for separate members; Per item may automatically select the shared Ticket owner as the displayed debtor, while the payment still explicitly names that member. The server never falls back to a Ticket, receipt, or bill owner. **Every method is available on every payment**, whatever the struk was already paid with: a bill-wide tender lock once collapsed the choice to the first method used and is gone. A payment may not exceed its receipt's outstanding.

A **refund names the leg it unwinds** — a `paymentId`, not a method (ADR-0121); the method is inherited and the amount capped at that leg's unreversed remainder, because two Tunai legs on one struk cannot be told apart by their method. A `piutang` leg unwinds as a [[Piutang]] **pembatalan** and no rupiah moves, which is the same rule ADR-0098 wrote as "piutang is not a refund method" — you still cannot hand drawer cash back for a promise. _Avoid_: treating a Payment as gateway-verified; attaching payments to the whole bill instead of a receipt; storing tendered cash as revenue; confusing the sales cash a Tunai payment brings in with the [[Kas kecil (petty cash)]] box, which only ever pays money out; reading a `piutang` payment as money received; refunding by method rather than by leg.
...
**ID · EN** — Kas kecil · Petty cash. Movements: isi kas · Top-up; pengeluaran · Expense; opname kas · Cash count; pembatalan · Reversal; pindah kas · Transfer. What an expense was for is a [[Kategori kas (cash category)]], **venue-authored** and owned by the [[Kas (cash box)|box]] it is spent from (ADR-0135). Balances stay `Rp 250.000` in both languages (ADR-0084).
...
**Petty cash is not revenue** (ADR-0089). It never enters `netTotal`, `settledTotal`, Bruto, or the payment mix, and reads only through its own **Kas** section in [[Report freshness (Live vs Snapshot)|Reports]] — opening balance, in, out, by category, closing balance — where the venue-wide by-category breakdown is keyed by **the venue's own word** so two tins that both authored *Sayur* read as one line (ADR-0135), and the opening balance respects `businessDayStartHour` so a 02:00 expense buckets with the sales it sat beside. An expense is not negative revenue and a top-up is not income.
...
The fund is kept in **one or more [[Kas (cash box)|kas]]** (ADR-0131) — named tins, each answering for its own balance.

_Avoid_: calling it the drawer, or reviving `openDrawer` for it; storing the balance; editing or deleting a movement; letting a would-be negative balance persist as a signal; folding a top-up into revenue or an expense into a write-off; linking an expense to a [[Mutasi stok (Stock movement)|stock receipt]] (correct end state, deliberately not yet — a wrong link corrupts two ledgers instead of one); giving the box an open/closed lifecycle; **one venue-wide balance**, or a guard checked against it; confusing it with a [[Pengeluaran kunjungan (visit expense)]], which is a different ledger, is revenue-affecting, and is spent by a waiter rather than a supervisor; a closed set of categories, or one list shared by every box.

### Kas (cash box)
**ID · EN** — Kas · Cash box. One named tin inside [[Kas kecil (petty cash)]] — *kas kecil* is the ledger and the fund, *a kas* is one of the containers it is kept in. Movement of money between two of them: **pindah kas** · Transfer. _Avoid_: "kotak kas" (nobody says it); "laci" (that is the drawer, ADR-0089); translating a box's name.

A venue keeps at least one and may keep several — Kas Utama for the owner, Kas Dapur for the morning market, one at the bar. Every venue starts with **Kas Utama**, which is also the box every movement written before ADR-0131 belongs to.

**The name is venue content**, like a zone's or a menu item's: authored by the owner, never an ARB key, never translated, and never a code — as is true of the [[Kategori kas (cash category)|categories]] it owns, which stopped being codes in ADR-0135. A box is **retired, never deleted** — a closed month must still be able to name where its money came from — and retiring is **refused while it holds money**, so hiding a tin from the picker can never hide rupiah with it.

**Each box answers for itself.** Its balance is `SUM(delta)` over its own movements, and ADR-0088's refusal is checked against that box alone: a full Kas Utama does not make an empty Kas Dapur spendable, because the notes are in another room. The venue total exists only as the sum of the boxes, and never as a figure a guard reads.

A **pindah kas** moves money between two boxes: an expense out of the source paired with a top-up into the destination, written together and linked. It is not a purchase — it carries no category and never reaches the report's by-category breakdown — and it is not two acts: one audit line, and reversing either leg undoes both. Funding a tin is the owner's authority (`editSettings`), not the supervisor's.

Spending is `manageCash` and funding, counting, transferring and managing the boxes is `editSettings`, **venue-wide** — there is no per-box permission. In [[Report freshness (Live vs Snapshot)|Reports]] the Kas block's every venue figure is the sum of its boxes, which is why a transfer disappears from the totals without a rule to exclude it. The day's [[Buka kedai / Tutup kedai (venue day)|opening and closing ritual]] funds and counts the **first active box** only.
...
What money left a [[Kas (cash box)|kas]] **for** — the one thing a [[Kas kecil (petty cash)|pengeluaran]] carries besides its amount, its note and its photo. **The box owns its categories** (ADR-0135): Kas Dapur lists what a kitchen tin buys, the bar tin lists its own, and neither offers the other's.

**The word is venue content**, like a box's name or a menu item's — authored by the owner, never an ARB key, never translated, never a code. The five stock words are seeded rows, not code: a venue renames *upah harian* to whatever it actually says, and the rename is **retroactive** — an entry stores the category, not the word, so last month's report re-reads under the new one. The [[Audit]] trail does not move, because a movement's audit row wrote the word at the time.

A category is **retired, never deleted** — a removed one orphans every expense filed under it and the report then renders an id where a word should be. A box may be retired down to **no** active categories; its expense sheet then says so and points at the editor, and the write path refuses. Authoring, renaming and retiring are `editSettings`, beside the boxes themselves on **`/kas`**; spending under one stays `manageCash`. A **pindah kas** carries none — nothing was bought.
...
Capturable offline on the [[Antrean setelmen]] — as its own event, not as money: one append-only row is not a chain, is never read back, and never reaches `recomputeBill`. It was an [[Antrean kirim]] intent until ADR-0139 merged the two stores; its photo already lived in the client database (ADR-0124), which is half of why it moved. A capture with **no cached subtotal is refused**, because a cap you can escape by turning off Wi-Fi is not a cap. At drain the cap is the only refusal that can fire, and like a void's it must not stall the queue.
...
A **durable person** the venue recognises across [[Visit|visits]] — name, phone, join date, optional note and birthday. The **member profile** is those identity and contact facts, plus optional [[Alamat pelanggan|address]] and credit limit; [[Poin|points]], [[Kartu stempel (punch card)|punch progress]], visit history and [[Piutang|debt]] are separate records, never profile fields. **The phone number is the identity** (ADR-0092): it is unique venue-wide, enrolling on an existing number attaches to that member rather than minting a second, and there is **no anonymous member** — a guest who will not give a number simply is not one. Duplicates still arrive by typo, so **merge** exists as an admin act (points sum, punch progress takes the max, both audited). A member is **venue-local** (ADR-0091) — it lives in the venue's own database, never in the cloud, so a lookup cannot fail because the internet did.

**Bulk enrolment creates new members only.** A phone already held by the directory is left untouched and reported; repeated forms of the same normalised number within one batch are invalid rather than resolved by row order. _Avoid_: treating bulk enrolment as a bulk edit; letting the last duplicate silently win.

**Distinct from `guestName`.** The waiter's party label on a [[Visit]] ("Pak Budi, 4 orang") is a *per-visit* string; the Pelanggan is the person behind it. Attaching a member **fills an empty `guestName` and never overwrites** one a waiter typed — the floor recognises the waiter's label, not the CRM's. A member may be the visit's [[Pemilik tagihan]], a line's [[Pemilik tiket]], or both. Under `memberSplit`, the bill-wide member gesture is only a shortcut that gives every Ticket which already exists the same owner; it creates neither another attribution grain nor a default for later Tickets.
...
The whole feature is off until **`membersEnabled`**; enroll, bill/receipt attach and [[Tukar poin|redeem]] ride the **`settleBill`** [[Capability]], while `takeOrder` may read only minimal member identity — searching for one, or resolving a set of ids it already holds so a sent line can name its owner on the floor — and name a [[Pemilik tiket]] on a CartItem under `memberSplit`. Edit, hand-adjustment, merge and delete need **`manageMembers`** — as does reading one member's visit history. Reachable at **`/members`**, tablet-only from the Venue hub, like [[Audit]] and [[Kas kecil (petty cash)]]. **Deleting a member anonymises, it never erases money** (ADR-0092): the person's row goes, the closed [[Bill (tab)|bills]] keep their `memberId` and render as "Pelanggan dihapus". _Avoid_: exposing history, points, debt, address or profile editing through order-taking; enrolling from a CartItem picker; an anonymous or walk-in member; overwriting `guestName`; deleting a member by deleting their settled history; storing a member in the cloud; a lifetime figure and a window figure side by side without saying which is which; a stored "lapsed" or "churned" status.
...
The [[Pelanggan (member)]] a whole [[Bill (tab)]] belongs to — `visits.memberId`, the one member a bill has had since membership shipped. The owner is the party's identity **on the floor**: they fill an empty `guestName`, they arrive with a [[Reservation]] when one was booked against them, and they print in the bill struk's header.

A bill always has exactly one owner or none; it never has two. Under `memberSplit`, the owner does **not** automatically own every line: consumption and its loyalty attribution belong to each [[Pemilik tiket]]. Outside that mode the established single-owner behavior remains. _Avoid_: treating the owner as merely "receipt zero"; using bill ownership as a hidden Ticket fallback while split attribution is active.

### Pemilik tiket
**ID · EN** — Pemilik tiket · Ticket owner.

The [[Pelanggan (member)]] one Ticket line belongs to: the person who consumed that item, independent of who pays its receipt, live only at a venue holding the **`memberSplit`** mode. One Ticket has at most one owner even when its quantity exceeds one; items for different members must be separate Ticket lines. A member may be chosen on the CartItem before send, and `memberId` is part of cart stacking identity so otherwise identical items for different people become different Tickets. Points, purchase history, and the member tier follow this owner; the tier becomes a line-scoped discount on each eligible Ticket, regardless of its payer or receipt. A bill-wide assignment is only a shortcut that assigns the same owner to every Ticket present at that moment, and later Tickets begin unassigned unless their own CartItem names one. An unassigned Ticket stays unattributed, does not block settlement, and gives nobody loyalty benefits or purchase history. Ownership stays editable regardless of kitchen status until the receipt containing the Ticket is paid; payment freezes it, and reopening that receipt unlocks it for correction. _Avoid_: quantity-level ownership inside one Ticket; assigning a whole [[Batch (kitchen order)|Batch]] as an indivisible unit; deriving the owner from the payer, [[Pemilik struk]], or [[Pemilik tagihan]]; applying one member's tier to another member's Ticket; treating bill-wide assignment as a default for future lines; previewing the settlement-only tier in the cart; reassigning a paid Ticket without reopening its receipt.

### Pemilik struk
**ID · EN** — Pemilik struk · Receipt owner.

A **retired** label: the optional [[Pelanggan (member)]] on one **receipt** — `receipts.memberId`. **Nothing writes one since ADR-0126.** It named the share on screen and paper and controlled neither loyalty, which follows [[Pemilik tiket]], nor [[Piutang]], whose debtor is explicit on each payment leg — which left it a second gesture that looked like "whose is this" and answered a different question. Asking twice is how the wrong guest gets the points.

Stored rows are **read, never rewritten**: a [[Bill (tab)]] that closed under ADR-0118 still reports and prints its receipt owners, and `memberUnitsOf` still divides its lines by them. Every bill since carries null, and the slip's owner list renders empty. A `piutang` debtor is confirmed on the payment leg, and who consumed a line is the [[Pemilik tiket]] chip on the line itself. _Avoid_: reviving a receipt-scope member picker; treating a null owner on a recent bill as missing data; backfilling stored owners onto Tickets.
...
Under the **`memberSplit`** mode the moment is unchanged and only the *grain* moves (ADR-0125): bill close groups eligible Ticket bases by [[Pemilik tiket]], floors once per member for the whole bill, and writes one earn row per member. The base is that member's lines net of every allocated discount and excludes service and tax; unassigned and voided Tickets earn nothing. A reopen reverses every row the close wrote before a re-close recomputes them.

**Points never expire** (ADR-0095) — the venue server has no scheduler, and a balance that changes by being looked at is worse than a growing one. The honest substitute is **Poin beredar** in the members Reports section: the outstanding balance read as a **liability**. _Avoid_: storing a balance; earning per payment, receipt, or Ticket rather than once per member per bill; falling back from an unassigned Ticket to the bill owner; earning on service and tax; expiring points on read; a negative balance.

### Tukar poin (redeem)
**ID · EN** — Tukar poin · Redeem.

Spending a [[Poin]] balance as money off a live [[Bill (tab)]], at the till, under `settleBill`. Under `memberSplit`, the cashier chooses one [[Pemilik tiket]] and redemption is distributed only across that member's editable Tickets, capped by their net value; it never discounts another member's food. It lands in the `redeem` source slot at the owner-set value (default 1 poin = Rp 1.000), minimum 10 poin, and redeemed rupiah earns nothing back. A redemption is never auto-reversed by a reopen; an audited hand-adjustment unwinds the rare mistake. _Avoid_: redeeming against unassigned or another member's Tickets; auto-reversing on reopen; letting redemption push a target below zero.
...
An owner-run "buy N, get one free" program on **one menu item at a time**, live only when **`memberPunchEnabled`** is on. Progress is **derived from settled history** — paid, non-voided, non-[[Comp|comped]] units of that item on closed bills — never a stored counter, for the same reason the cash balance is not stored. The **reward is a [[Comp]]**, offered at settlement and booked through the existing comp path, so it reports as a write-off rather than as revenue and is already audited; **the free unit does not count** toward the next card. Takeaway counts — same guest, same item. Under `memberSplit`, every qualifying unit belongs to its Ticket's [[Pemilik tiket]]; a quantity line cannot split between members, so different owners require different Tickets. _Avoid_: many concurrent programs; a stored punch counter; booking the reward as a discount; counting comped, voided, or unassigned units; deriving ownership from a receipt or bill.

### Penanggung utang (debtor)
**ID · EN** — Penanggung utang · Who owes this.

The [[Pelanggan (member)]] responsible for the debt recorded by one [[Piutang]] payment. For Per item, the shared [[Pemilik tiket]] of all selected items is automatically assigned as the debtor; the cashier may choose someone else before confirming payment. An automatic choice follows the selected items and clears when their owners are mixed or any item has no owner; a deliberate picker choice remains for the current payment.
...
The ledger is **append-only** and its balance is `SUM(delta)` — **derived, never stored**, the same posture as [[Poin]] and [[Kas kecil (petty cash)]]. Five movements: **tagih** (positive, written against the member explicitly named on that `piutang` payment leg), **terima** (negative, a collection carrying its own method and — for anything but cash — a [[Payment proof photo (Bukti pembayaran non-tunai)|proof photo]]), **pembatalan** (automatic, when the receipt that raised the charge is reopened, or when that charge's leg is refunded — partial, so a part-refund then a reopen nets to exactly the charge), **hapus buku** (giving up on collecting) and **koreksi** (a hand correction with a mandatory reason). One receipt may therefore raise separate charges for separate members through separate payment legs. The last two are separate on purpose: once a visit is snapshotted there is nothing left to reopen, so without **koreksi** a typo could only be fixed by a **hapus buku**, and the bad-debt figure would mix real losses with corrections.
...
Under `memberSplit`, spend and products divide by [[Pemilik tiket]] (ADR-0125): each distinct owner appears once for a closed bill, their products contain only their non-voided Tickets, and shared bill/receipt discounts plus service and tax are allocated proportionally so all member and unassigned shares still reconcile to the settled total. Unassigned Tickets remain venue sales but appear in no member's history. Outside the mode, the existing [[Pemilik tagihan]] reading is unchanged. **Belanja tanpa rincian** remains a legacy ADR-0118 figure only; new Ticket-attributed visits cannot assign member spend with no owned Ticket.
...
### Export (report / order history / staff / accounting / membership)
**ID · EN** — Ekspor · Export; Jenis · Kind. Kinds: Laporan · Report; Pesanan · Orders; Staf · Staff; Akuntansi · Accounting; Daftar pelanggan · Member directory; Daftar peringkat · Ranked list; Riwayat pelanggan · Member history. CSV headers and PDF section titles follow the exporting device's locale (ADR-0083); the amounts inside them do not (ADR-0084).

A **range-scoped** download of venue data as **CSV or PDF**, generated on-device and handed off through the Android **share sheet** (`share_plus`). The four venue-wide kinds share one entry point on the [[Reports]] screen — a single **Ekspor** action opens one sheet where the user first picks a **Jenis** (kind): **Laporan**, **Pesanan**, **Staf**, or **Akuntansi**. All four read the **same active timeline chip** (including [[Custom range]]); there is no per-export range picker. The four kinds:
...
- **Pesanan (order history export)** — Closed visits in the selected report window, grouped by table/party with the visit waiter, zone and settled total. Each item names its [[Orderer (line author)|orderer]], [[Guest note / Item note|item note]] and [[Pemilik tiket|member consumer]] independently of the payer. Variant belongs in the item description; course is omitted. Direct deductions and proportionally allocated shared discounts are separate from the named offers and the before/after-discount item amounts. Shared allocations reconcile to recorded totals; incomplete history shows unavailable item amounts and an unallocated visit discount. [[Void (item)|Voided]] lines remain flagged with their reason and contribute zero to after-discount totals. Receipt, payment and proof details remain part of the report. An unassigned consumer differs from an unavailable historical identity.
...
Three further kinds belong to membership and live on **their own screens**, not in that sheet — see [[../docs/adr/0137-an-export-is-a-fresh-uncapped-fetch|ADR-0137]]:

- **Daftar pelanggan (member directory export)** — the [[Pelanggan (member)]] roster as it stands, **CSV only** (a roster has one useful shape, so there is no format picker), from a plain **Ekspor CSV** action beside **Impor CSV** on `/members`. Carries the screen's active search / birthday / **belum kembali** filters and names them in its header, because "the members who have not come back in ninety days" is the reason an owner takes one. Leads with the nine columns the **importer** reads, in import order, then the derived tail (kode, [[Poin]], stempel, [[Piutang]], kunjungan, total belanja) — a courtesy to whoever opens both files, explicitly **not** a round-trip promise.
- **Daftar peringkat (ranked list export)** — everyone who traded in the [[Laporan pelanggan]] window, the summary block then the table, CSV or PDF. Carries the search box and the active sort as the list has them.
- **Riwayat pelanggan (member history export)** — one member's bills and product rollup for that window, CSV or PDF. Answers for a **deleted** member, like the screen does.

Generated via **dedicated read-only server endpoints** scoped to the range: **Laporan** reuses the [[Reports]] snapshot already in memory; **Pesanan**, **Staf**, and **Akuntansi** each hit a purpose-built window query (`/reports/staff`, `/reports/accounting`) that reuses the snapshot's `_resolveRange`; the three membership kinds hit `/members/export`, `/members/report/export` and `/members/<id>/report/export`.

An export is **a fresh, uncapped fetch, never what the screen is holding** — every membership read is server-capped (the directory and the ranked list at 500, a member's bills at 200), so a file built from loaded state stops where the reader stopped and calls itself complete. "Uncapped" is still bounded: past **20 000 rows** an export **refuses loudly** (`export_too_large`, naming the limit) rather than truncating.

The four venue-wide kinds gate `viewReports` — even the order list (otherwise open to `takeOrder`) — because export exposes historical financial data. The membership kinds gate **exactly what their screen gates** (`manageMembers` for the directory; `viewReports` **or** `manageMembers` for the two reports), because there they widen nothing a reader could not already see. The directory export is the one that narrows: `manageMembers` alone, unlike `GET /members`, which the till also reads — which is also why no masked device (ADR-0129) ever reaches an unmasked file. The directory export writes an audit row; no other export does.

_Avoid_: turning the live order board into a historical browser; trusting a client to widen its own range past the gate; splitting the venue-wide kinds into separate header buttons (one **Ekspor** entry, Jenis chosen inside the sheet); letting the on-screen tax figure and the accounting one come from two different sums; building a file from a capped list; exporting the [[Salinan pelanggan]] when the host is dark.
...
A session **names the person who walked it**, and the name is frozen onto the header at write — never resolved live on read, for the reason a line freezes its bahan name and its cost: a filing copy printed a year later must name whoever counted, not whoever holds that user row today. Who **opened** it and who **closed** it are two facts, because a manager may finish somebody else's walk; when they are the same person the document says so once. Attribution rides every surface the session appears on — the archive tile, the document header, the PDF filing copy and the CSV — and a session with no attribution at all renders **Tidak tercatat** rather than a blank, because an unattributed stocktake is a finding.

Closing writes the movements, stamps who closed it, and posts **one** row to the [[Audit (venue audit log)|audit log]] — the session, not its lines. _Avoid_: an opname as a transient screen mode (a 40-minute count dies with the tablet's screen); dropping the zero-variance line; valuing a historic session at today's cost (the archive would rewrite itself); a sighted count presented as equal evidence to a blind one; resolving the counter's name at read time (a rename or a staff delete would then rewrite the archive); per-line attribution (it measures the counter shelf by shelf, which is how numbers start getting fudged).
...
_Avoid_: reading the slot's absence as a permission error the way a refusal reads — a person holding neither stock capability simply has no such destination (ADR-0134); hiding the slot from an admin on the grounds that they already have the hub tile; badging it with the low count.

### Venue hub
**ID · EN** — Venue · Venue.

The **catalogue of the venue's back-of-house destinations** — a grid of tiles, each one a screen with its own authority. It is deliberately *not* an authority itself: it opens to whoever can open at least one thing inside it, and the tiles filter to exactly those (ADR-0134). Gating the room on `manageStaff` is what made the [[Stok (nav destination) (Stock)|stok]], [[Kas kecil (petty cash)|kas]], [[Opname (Stocktake)|opname]] and [[Reports|laporan]] gates unreachable by the people they name.
