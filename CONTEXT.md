...
A deliberate, authorized reduction of what a guest owes, applied by the [[Cashier]] at [[Settlement (recording payment)|settlement]] and **never** during order-taking. Three targets, one mechanism: a **bill** discount against the whole [[Bill (tab)]], a **whole-order** discount against one receipt, or a **line** discount against one bill line. The bill scope is what a table-wide promo actually is — it reduces the bill subtotal before any receipt claims anything, then distributes across receipts the way an order discount already does, so "20% off the table, split three ways" is one act rather than one per payer. See [docs/adr/0070-discounts-have-a-bill-level-scope.md](docs/adr/0070-discounts-have-a-bill-level-scope.md). Every target carries a **[[Sumber diskon (discount source)|source]]** (`manual` | `member` | `redeem`). Bill and receipt targets may stack one row per source; a line accepts exactly one reduction, so an automatic [[Pemilik tiket]] member tier locks out a manual line discount. The member tier is always line-scoped and percentage-based; a fixed member preset cannot be enabled under `memberSplit`, because repeating a fixed amount per Ticket silently multiplies the benefit. Applied rows snapshot their name, kind, and value, so later preset edits never rewrite settled history. Free to add, swap, and remove while the containing receipt is unpaid; **frozen once paid**, corrected only by audited [[Settlement (recording payment)|reopen]]. Manual discounts remain gated by `applyDiscount` or manager step-up; automatic member discounts ride `settleBill`. A discount can never push a total below zero and line discounts remain pre-tax. _Avoid_: stacking a manual and member discount on one line; multiplying a fixed member benefit per Ticket; letting a payer's tier replace the Ticket owner's.
