...
Its total is the tapped lines grown by the bill's own effective [[Tax & service charge|service and tax]] rate — the same figure the confirm button shows, from one shared rule, so the two can never disagree. It carries **no [[Diskon (discount)]]**: a give-back attaches to a receipt and there is none yet, so a discount applied afterwards only makes the guest's real share smaller than the quote. _Avoid_: reading one as a minted [[Split bill|receipt]]; giving it a [[Receipt letter]]; expecting N of them to sum to the bill total to the rupiah (each rounds on its own — they are quotes, and the minted receipts are what the arithmetic closes on); minting an unpaid receipt to get a printout.
...
A split answers **which receipt pays for which claim**. At a venue holding the **`memberSplit`** mode, each [[Pemilik tiket]] independently answers who consumed a line while an optional [[Pemilik struk]] names the share on paper; a `piutang` [[Payment (manual confirmation)|payment]] separately names who accepted that debt. The facts may differ deliberately: Ani may pay a receipt containing Budi's Tickets without taking Budi's loyalty attribution. _Avoid_: deriving consumption or debt responsibility from the receipt label.
...
A [[Pemilik struk]] is the one identity an amount receipt may hold, and only because it is **declared** rather than derived (ADR-0125): a letter is read off the lines a receipt owns, which an even share has none of, whereas a member is named by a [[Cashier]] to label the share. The moment one is named the two shares stop being interchangeable, and the share prints that member's name in place of `Bagian 1/3`. _Avoid_: reading this as licence to letter an even share, as loyalty attribution, or as the debtor on a `piutang` payment.
...
A deliberate, authorized reduction of what a guest owes, applied by the [[Cashier]] at [[Settlement (recording payment)|settlement]] and **never** during order-taking. Three targets, one mechanism: a **bill** discount against the whole [[Bill (tab)]], a **whole-order** discount against one receipt, or a **line** discount against one bill line. The bill scope is what a table-wide promo actually is — it reduces the bill subtotal before any receipt claims anything, then distributes across receipts the way an order discount already does, so "20% off the table, split three ways" is one act rather than one per payer. See [docs/adr/0070-discounts-have-a-bill-level-scope.md](docs/adr/0070-discounts-have-a-bill-level-scope.md). Every target carries a **[[Sumber diskon (discount source)|source]]** (`manual` | `member` | `redeem`), and uniqueness is one row per target **per source**: a cashier promo, a [[Pemilik tiket]]'s percentage member tier, and a [[Tukar poin (redeem)|redemption]] may stack while two manual promos on the same target may not. The member tier is always line-scoped and percentage-based; a fixed member preset cannot be enabled under `memberSplit`, because repeating a fixed amount per Ticket silently multiplies the benefit. Applied rows snapshot their name, kind, and value, so later preset edits never rewrite settled history. Free to add, swap, and remove while the containing receipt is unpaid; **frozen once paid**, corrected only by audited [[Settlement (recording payment)|reopen]]. Manual discounts remain gated by `applyDiscount` or manager step-up; automatic member discounts ride `settleBill`. A discount can never push a total below zero and line discounts remain pre-tax. _Avoid_: two discounts of the same source on one target; multiplying a fixed member benefit per Ticket; letting a payer's tier replace the Ticket owner's.
...
The optional [[Pelanggan (member)]] label on one **receipt** — `receipts.memberId`, live only at a venue holding the **`memberSplit`** mode (ADR-0125). It names the share on screen and paper; it controls neither loyalty, which follows [[Pemilik tiket]], nor [[Piutang]], whose debtor is explicit on each payment leg.

Receipt labeling and ticket ownership are independent: the receipt partitions what is paid, while each [[Pemilik tiket]] identifies who consumed a line and receives its loyalty attribution. A receipt label never overrides a Ticket owner or supplies a debtor. Both kinds of receipt may hold one, an [[Amount receipt]] included (see there: declared, never derived), and the member's name replaces the share's label on the slip.

It is **declared once and frozen with the money** — set while the receipt is unpaid, locked at its first payment along with everything else on it (ADR-0068), corrected only through the audited [[Settlement (recording payment)|reopen]]. It is a **weak reference**, like the owner's: deleting a member leaves it dangling on purpose, because the receipt *was* theirs. Switching the mode off freezes rather than deletes stored owners. _Avoid_: a receipt owner instead of a bill owner; using receipt ownership as loyalty attribution; re-attributing a paid receipt without reopening it.
