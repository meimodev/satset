...
### Export (report / order history)
A **range-scoped** download of venue data as **CSV or PDF**, generated on-device and handed off through the Android **share sheet** (`share_plus`). One entry point on the [[Reports]] screen — a single **Ekspor** action opens one sheet where the user first picks a **Jenis** (kind): **Laporan** or **Pesanan**. The two kinds:
...
Generated **client-side** for both: the [[Reports]] snapshot is already in memory; order history comes from a dedicated read-only server window query. Gated behind `viewReports` even on the order list (which is otherwise open to `takeOrder`), because export exposes historical financial data. _Avoid_: turning the live order board into a historical browser; trusting a client to widen its own range past the gate; splitting the two kinds into two separate header buttons (one **Ekspor** entry, Jenis chosen inside the sheet).
