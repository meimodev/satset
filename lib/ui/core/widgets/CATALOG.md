...
| `context.shellInset` | `shell_inset.dart` | Clearance a vertical scroll view owes the shell's floating phone tab bar; **0** on the tablet shell and on every root-navigator push. Published by `AppShell`, never derived from `MediaQuery` — a screen mounted both inside the shell and pushed outside it (`MenuScreen`, `ReviewScreen`) needs a different answer at the same width. ADR-0117. |
...
| `UpdateBanner` | `update_banner.dart` | "A newer build is out" nag. `recommended` only, Main Device only — the one update surface that interrupts (ADR-0130, ADR-0131). |
| `UpdateBlock` | `update_block.dart` | Mandatory-update cover. Non-dismissible layer above the router; wrap the router child. Installs on **every** device, ungated (ADR-0131). |
| `startUpdate()` | `update_action.dart` | Not a widget — the one entry into the installer from the UI. Warns with the live-table count on the host, whose install stops the server (ADR-0131). Banner, block and the `/me` version line all go through it. |
...
| `ProofViewer` | `payment_proof_thumb.dart` | The fullscreen proof lightbox — `satMediaChrome` ground, pinch-zoom, ADR-0082's chrome. `PaymentProofThumb` pushes it on tap; the venue log pushes it from a row's camera glyph, which has no thumb to tap (ADR-0086). **Takes bytes, never an id** — fetching is the caller's business, so the viewer has one job and no loading state. |
| `shootProofPhoto()` | `proof_photo.dart` | Every proof capture — the one place `ImagePicker` is configured (camera, 1080 x 1080, quality 80). Four call sites wanted the identical four arguments: the two settle panes, the [[Piutang]] collection sheet, and the [[Pengeluaran kunjungan]] sheet, and four copies of a compression setting is four places for a proof to quietly become a different size. Not a widget — a function — but it belongs here for the same reason the thumb does. **It does not swallow a failure**: on the one screen where the photo is mandatory (ADR-0130) a silent null reads as "changed their mind" and leaves nothing to show the user. |
...
| `FloorStaleBanner` | `floor_stale_banner.dart` | One-line notice that the floor is this device's [[Salinan lantai]], stamped with when it was written. Renders only when the copy is what is painted *and* the host is away (ADR-0133). |
