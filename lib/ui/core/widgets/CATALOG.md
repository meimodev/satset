...
| `SatTabs` + `SatTab` | `sat_tabs.dart` | Segmented strip. No `TabController` — the screen already holds the index. For a handful of options that all fit, prefer a `SatChip.select` row, which shows every choice at once. Two tabs is the **scope switch** shape — mutually exclusive views of one list (the Pesanan board's Milik saya / Semua, ADR-0056) — and needs no wrapper widget of its own. |
