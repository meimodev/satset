...
Router reservationsRoutes(AppDatabase db, WsHub hub, [ServerAuth? auth]) {
  ...
  r.get('/reservations', (Request req) async {
    ...
    return Response.ok(jsonEncode([for (final x in rows) _toJson(x)]),
        headers: {'content-type': 'application/json'});
  });
  ...
  r.post('/reservations', (Request req) async {
    ...
    if (name.isEmpty) {
      return Response(400, body: jsonEncode({'code': 'bad_request', 'message': 'name required'}), headers: {'content-type': 'application/json'});
    }
    final expectedAtRaw = body['expectedAt'] as String?;
    if (expectedAtRaw == null) {
      return Response(400, body: jsonEncode({'code': 'bad_request', 'message': 'expectedAt required'}), headers: {'content-type': 'application/json'});
    }
    final id = uuid.v4();
    ...
    final row = await (db.select(db.reservations)..where((t) => t.id.equals(id))).getSingle();
  ...
  r.patch('/reservations/<id>', (Request req, String id) async {
    ...
    if (statusRaw != null && !_allowedStatuses.contains(statusRaw)) {
      return Response(400, body: jsonEncode({'code': 'bad_request', 'message': 'invalid status'}), headers: {'content-type': 'application/json'});
    }
    await (db.update(db.reservations)..where((t) => t.id.equals(id))).write(
      ReservationsCompanion(
        name: body.containsKey('name') ? Value((body['name'] as String).trim()) : const Value.absent(),
        phone: body.containsKey('phone') ? Value((body['phone'] as String?)?.trim()) : const Value.absent(),
